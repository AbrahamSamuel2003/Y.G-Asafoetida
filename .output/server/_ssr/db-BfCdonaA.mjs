...
//#region node_modules/.nitro/vite/services/ssr/assets/db-BfCdonaA.js
		...
		if (upper.startsWith("INSERT")) {
			...
			return {
				...
				run: (...params) => {
					...
					let existingIdx = -1;
					if (tableName === "products" && obj["slug"]) existingIdx = tableData.findIndex((r) => r["slug"] === obj["slug"]);
					else if (tableName === "product_variants" && obj["product_slug"] && obj["id"]) existingIdx = tableData.findIndex((r) => r["product_slug"] === obj["product_slug"] && r["id"] === obj["id"]);
					else if (tableName === "promos" && obj["code"]) existingIdx = tableData.findIndex((r) => r["code"] === obj["code"]);
					else if (obj["id"]) existingIdx = tableData.findIndex((r) => r["id"] === obj["id"]);
					if (existingIdx >= 0 && tableData[existingIdx]) tableData[existingIdx] = {
						...tableData[existingIdx],
						...obj
					};
					else tableData.push(obj);
					this.save();
					return { changes: 1 };
				}
			};
			...
			return {
				...
				run: (...params) => {
					...
					for (const row of matching) setFields.forEach((field, i) => {
						if (field) row[field] = setParams[i];
					});
					this.save();
